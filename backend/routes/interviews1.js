...
router.post('/', function(req, res, next) {
  ...
  p1=async()=>await new Promise(function(resolve,reject){return Interviews.find({$or:[
...
//p1
  ...
  p2=async(data)=>await new Promise(function(resolve,reject){
    let result=[];
    if(data.length){
      result = data.map(a => a._id.toString());
      console.log(result);
      await UserInterviews.find({$and:[{interview:{$in:result}},{email:{$in:req.body.selectedCandidates}}]})
      .then(data=>{
        if(data.length>0){
  ...
  p3=async(data)=>await new Promise({function(resolve,reject){
    if(data===1){
      await Interviews.create({startTime:req.body.startTime,endTime:req.body.endTime})
      .then(data=>{
        return data._id;
      },(err)=>next(err))
      .catch((err)=>next(err))
  ...
  p4=async(id)=>await new Promise({function(resolve,reject){
